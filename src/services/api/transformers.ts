...
const normalizeModelAliases = (models: unknown): ModelAlias[] => {
  ...
  return models
    .map((item) => {
      ...
      const maxContextLength = item['max-context-length'];
      const inputModalities = item['input-modalities'];
      ...
      if (maxContextLength !== undefined && maxContextLength !== null) {
      ...
      if (Array.isArray(inputModalities)) {
        ...
        const normalizedModalities: string[] = [];
        inputModalities.forEach((modality) => {
          const trimmed = String(modality ?? '').trim();
          if (!trimmed || seen.has(trimmed.toLowerCase())) return;
          seen.add(trimmed.toLowerCase());
          normalizedModalities.push(trimmed);
        });
        if (normalizedModalities.length) {
          entry.inputModalities = normalizedModalities;
        }
      }
      if (thinking) {
